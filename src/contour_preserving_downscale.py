...
def preserve_contour(
    ...
    Args:
        ...
        dilate_factor: Dilated contour width in downscaled pixels.
        contour_mode: Contour dilateion mode. One of "precise", "darker", "dark", "lighter", "light".
    ...
    # Extract and apply contour mask
    ...
    if contour_mode == "precise":
        # Create a 2d Gaussian kernel, masked operation is not separable
        dilate_size = max(int(scale_factor * dilate_factor) // 2 * 2 + 1, 1)
        ...
        # Perform masked Gaussian smoothing
        ...
        contour_img = (img * (1 - dilated_contour_mask) + dilated_contour * (
            dilated_contour_mask
        ) + 0.5).clip(0, 255).astype(np.uint8)
    else:
        mode = "light" if contour_mode.startswith("light") else "dark"
        ...
        contour_windows_mono = padded_view_as_windows(
            contour_mono, (dilate_size, dilate_size), value=(255 if mode == "dark" else 0)
        )

        # Same for contour mask
        contour_mask_windows = padded_view_as_windows(
            contour_mask, (dilate_size, dilate_size)
        )

        # Find index of the darkest/lightest pixel among nearest neighbors
        if mode == "dark":
            ...
            index_min = np.argmax(contour_windows_mono * disk_kernel, axis=-1)
        j, k = np.indices(index_min.shape)

        # Dilate contour with pixels from these indices
        dilated_contour = contour_windows[j, k, index_min, :]

        # Darker and lighter modes use undilated contour pixels where possible
        if contour_mode.endswith("er"):
        ...
        # contour_img = dilated_contour.clip(0, 255).astype(np.uint8)
